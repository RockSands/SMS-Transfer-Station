...
public class OkHttpUtil {
	 ...
	 * @return String
	 */
	public static byte[] getForByte(String url, Map<String, String> header, Map<String, String> queries) {
		StringBuffer sb = new StringBuffer(url);
		if (queries != null && queries.keySet().size() > 0) {
			sb.append("?clientId=blade");
			queries.forEach((k, v) -> sb.append("&").append(k).append("=").append(v));
	 ...
	 * @return String
	 */
	public static String postContent(String url, Map<String, String> header,String name, String fileName, byte[] content, MediaType mediaType) {
		RequestBody requestBody = RequestBody.create(mediaType, content);
		Request.Builder builder = new Request.Builder();
		MultipartBody multipartBody = new MultipartBody.Builder().addFormDataPart(name, fileName, requestBody).setType(mediaType).build();
		if (header != null && header.keySet().size() > 0) {
			header.forEach(builder::addHeader);
		}
		Request request = builder.url(url).post(multipartBody)
				.build();
		return getBody(request);
	}
...
//        log.debug("url: {}\n headers: {}\n body: {}", request.url(), request.headers(), request.body());
        log.info("url: {}\n headers: {}\n body: {}", request.url(), request.headers(), request.body());
        String responseBody = "";
	 ...
	 * @return String
	 */
	private static byte[] getBodyForByte(Request request) {
